...
class ControllerPaymentideal extends Controller {
    ...
    */
    ...
    public function storeTxid ($method, $txid, $order_id) {
    	...
    	$sql = "INSERT INTO `".DB_PREFIX."ideal` SET ".
        	   ...
        	   "`ideal_txid`='".$this->db->escape($txid)."' ON DUPLICATE KEY UPDATE `ideal_txid`='".$this->db->escape($txid)."', ideal_response=null";
    	$this->db->query ($sql);
    	}
    ...
    */
    ...
    public function getTxid ($order_id) {
    	...
    	$sql = "SELECT * FROM `".DB_PREFIX."ideal` WHERE `order_id`='".$this->db->escape($order_id)."'";
